...
def test_slice_out_of_bounds_skipped_with_warning():
    ...
    # out-of-bounds scan is skipped
    assert ctx.n_scan == 0
    assert ctx.slice_indices == []
    assert ctx.subpixel_shifts == []


def test_slice_out_of_bounds_strict_raises():
...
def test_mixed_valid_and_invalid_scan_positions():
    with pytest.warns(UserWarning):
        ctx = PtychoContext(
            scan_positions=[
                (0.0, 0.0),       # valid
